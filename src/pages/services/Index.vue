            ...
            <v-card-text>
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="editedItem.system_percentage" v-bind:label="$t('systempercentage')" :rules="percentageRules" required></v-text-field>
                  </v-col>
                </v-row>
                <v-row>
                  <v-col cols="12" sm="6" md="4">
  ...
  export default {
    ...
    computed: {
      ...
      headers () {
        return  [
          ...
          { text: i18n.t('systempercentage'), value: 'system_percentage' },
    ...
    methods: {
      initialize () {
        ...
        for(var i=0; i <  this.user.role.permissions.length; i++) {
          if (this.user.role.permissions[i].name.includes("CAN_ALL") || this.user.role.permissions[i].name.includes('CAN_UPDATE') || this.user.role.permissions[i].name.includes('CAN_DELETE')) {
             flag = false;
             console.log(this.user.role.permissions[i].name);
          }          
        }
        if (flag === true) {
        ...
        // fill data
        this.$api.get(`${process.env.VUE_APP_API_ENDPOINT}admin/service`, {
      ...
      deleteItem (item) {
        ...
        if (answer) {
          this.$api.delete(`${process.env.VUE_APP_API_ENDPOINT}admin/service/${item.id}`,
          {
            headers: {Authorization: 'Bearer ' + localStorage.getItem('token')}
          })
          .then(response => {
      ...
      save () {
        ...
        if (this.editedIndex > -1) {
          this.$api.patch(`${process.env.VUE_APP_API_ENDPOINT}admin/service/${this.editedItem.id}`,
          {
            name: this.editedItem.name,
            description: this.editedItem.description,
            system_percentage: this.editedItem.system_percentage
          },
          {
            headers: {Authorization: 'Bearer ' + localStorage.getItem('token')}
          })
          ...
          this.$api.post(`${process.env.VUE_APP_API_ENDPOINT}admin/service`,
          {
            name: this.editedItem.name,
            description: this.editedItem.description,
            system_percentage: this.editedItem.system_percentage
