<template>
  ...
  >
    ...
    <template v-slot:item.services="{ item }">
      <router-link :to="{ name: 'Customer_services', params: { id: item.id } }">{{$t('customerservices')}}</router-link>
    </template>
  ...
  export default {
    ...
    methods: {
      initialize () {
        ...
        // fill data
        this.$api.get(`${process.env.VUE_APP_API_ENDPOINT}admin/customer`, {
      ...
      editItem (item) {
        ...
        const answer = confirm('Are you sure you want to activate/deactivate this customer?')
        if (answer) {
          this.$api.post(`${process.env.VUE_APP_API_ENDPOINT}admin/customer/activate`,
          {
            id: item.id,
