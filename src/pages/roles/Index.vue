<template>
  ...
  >
            ...
            <v-card-text>
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" sm="6" md="4">
                    <select v-model="editedItem.permission_ids" multiple :required="true">
    ...
    <template v-slot:item.permissions="{ item }">
      <v-treeview :items="item.permissions"></v-treeview>
    </template>
  ...
  export default {
    ...
    computed: {
      ...
      headers () {
        return [
          ...
          { text: i18n.t('permissions'), value: 'permissions' },
    ...
    methods: {
      initialize () {
        ...
        // fill data
         this.$api.get(`${process.env.VUE_APP_API_ENDPOINT}admin/permission`, {
      ...
      deleteItem (item) {
        ...
        if (answer) {
          this.$api.delete(`${process.env.VUE_APP_API_ENDPOINT}admin/role/${item.id}`,
          {
            headers: {Authorization: 'Bearer ' + localStorage.getItem('token')}
          })
          .then(response => {
      ...
      save () {
        if (this.editedIndex > -1) {
          console.log(this.editedItem.permission_ids);
          this.$api.patch(`${process.env.VUE_APP_API_ENDPOINT}admin/role/${this.editedItem.id}`,
          {
            name: this.editedItem.name,
            permissions: this.editedItem.permission_ids
          },
          {
            headers: {Authorization: 'Bearer ' + localStorage.getItem('token')}
          })
          ...
          this.$api.post(`${process.env.VUE_APP_API_ENDPOINT}admin/role`,
          {
            name: this.editedItem.name,
            permissions: this.editedItem.permission_ids
          },
